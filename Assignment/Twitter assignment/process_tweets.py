...
for filename in os.listdir(loc):
    if filename.endswith(".json"):
        ...
        with open(loc+'/'+filename) as data_file:
            ...
            tm=time.strptime(cr)
            row.append(searchterm+','+userid+','+str(tm[0])+','+str('{0:02d}'.format(tm[1]))+','+str('{0:02d}'.format(tm[2]))+','+str('{0:02d}'.format(tm[3]))+','+str('{0:02d}'.format(tm[4]))+','+str('{0:02d}'.format(tm[2]))+','+str(data['retweet_count'])+','+str(data['favorite_count'])+','+str(data['user']['friends_count'])+','+str(data['user']['followers_count'])+','+str(data['user']['location'])[str(data['user']['location']).find(',')+1:]+'\n')
